package com.example.appadmindatvephim.Util;

public class Util {
    public static final String LINK_LOADMOVIE = "http://192.168.1.7:3000/loadphimadmin?soluong=5&vitri=%d";
    public static final String LINK_UPDATEMOVIE = "http://192.168.1.7:3000/updatemovie";
    public static final String LINK_SEARCHMOVIE = "http://192.168.1.7:3000/timkiemmovieadmin?tenphim=%s";
    public static final String LINK_ADDMOVIE = "http://192.168.1.7:3000/insertnewmovieadmin";
    public static final String LINK_DELETEMOVIE = "http://192.168.1.7:3000/deletemovieadmin";
    public static final String LINK_LOADTHELOAI = "http://192.168.1.7:3000/loadloaiphimadmin";
    public static final String LINK_LOADDATACUSTOMER = "http://192.168.1.7:3000/loadcustomeradmin";
    public static  final  String LINK_LOADDATACINEMA = "http://192.168.1.7:3000/loadcinemaadmin";
}